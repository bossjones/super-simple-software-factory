...
def sdk_event(event_type, data, *, event_id="00000000-0000-0000-0000-000000000001",
              agent_id=None, timestamp="2026-09-14T12:00:00+00:00"):
    return SessionEvent(
        type=event_type,
        id=UUID(event_id),
...
def test_tool_execution_is_folded_into_one_record():
    ...
    assert normalizer.observe(sdk_event(
    ...
    records = normalizer.observe(sdk_event(
...
def test_tool_record_uses_event_timestamps_and_clips_strings():
    ...
    record = normalizer.observe(sdk_event(
        SessionEventType.TOOL_EXECUTION_COMPLETE,
        ToolExecutionCompleteData(
            tool_call_id="call-1",
...
def test_missing_or_unpaired_completion_is_an_error_not_a_successful_tool_call():
    normalizer = CopilotEventNormalizer()
    missing_id = normalizer.observe(sdk_event(
        SessionEventType.TOOL_EXECUTION_COMPLETE,
        ToolExecutionCompleteData(
            tool_call_id="",
            result=ToolExecutionCompleteResult(content="contents"),
    ...
    unpaired = normalizer.observe(sdk_event(
        SessionEventType.TOOL_EXECUTION_COMPLETE,
        ToolExecutionCompleteData(
...
def test_partial_results_and_progress_are_folded_into_completed_tool_record():
    ...
    late_update = normalizer.observe(sdk_event(
        SessionEventType.TOOL_EXECUTION_PROGRESS,
...
def test_interruption_closes_open_tools_and_later_completion_cannot_duplicate():
    ...
    interrupted = normalizer.observe(sdk_event(
    ...
    duplicate = normalizer.observe(sdk_event(
        SessionEventType.TOOL_EXECUTION_COMPLETE,
...
def test_model_failure_and_abort_are_explicit_errors_without_reasoning():
    normalizer = CopilotEventNormalizer()
    model_failure = normalizer.observe(sdk_event(
    ...
    abort = normalizer.observe(sdk_event(
...
def test_unknown_events_are_ignored():
    assert CopilotEventNormalizer().observe(
        lightweight_event("future.event", secret="value")
    ) == []
